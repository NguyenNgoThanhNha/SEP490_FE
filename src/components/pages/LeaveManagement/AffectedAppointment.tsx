    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-3xl p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold">Lịch hẹn của nhân viên</h2>
          <Button variant="outline" size="sm" onClick={onClose}>
            Đóng
          </Button>
        </div>
        <p className="text-sm text-muted-foreground mb-4">
          Lịch nghỉ của nhân viên {request.staffName} ngày{" "}
                  ...
                  <div className="flex justify-between items-start">
                    <div className="space-y-2">
                      ...
                      <div className="text-sm text-muted-foreground">
                        Dịch vụ: {appointment.service.name} ({appointment.service.duration} mins)
                      </div>
                      <div className="text-sm text-muted-foreground">
                        Giá: {appointment.unitPrice.toLocaleString()} VND
                      </div>
                    </div>
                    <Button
                      variant="outline"
