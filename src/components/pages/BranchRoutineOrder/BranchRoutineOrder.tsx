...
const BranchRoutineOrder = () => {
    ...
    const fetchOrders = async (PageIndex: number, PageSize: number) => {
        try {
            ...
            const response = await orderService.getAllOrder({
                ...
                PageIndex,
                PageSize,
                OrderType: "Routine", // Chỉ lấy đơn hàng có orderType === "Routine"
            });
    ...
    const handleEdit = (routineId: number, userId: number) => {
        navigate(`/user-routine-step/${routineId}/${userId}`);
    };
                    ...
                    actions={(row) => (
                        <>
                            <button className="text-blue-500 hover:text-blue-700"
                                onClick={() => handleEdit(row.routineId as number, row.customer.userId as number)} 
...
export default BranchRoutineOrder;
